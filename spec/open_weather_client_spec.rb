...
RSpec.describe OpenWeatherClient do
  ...
  describe '#check_weather' do
    ...
    it 'options the units to be metric on the request' do
    ...
    it 'returns a hash of weather infomation if successful' do
      weather_info_hash = JSON.parse(weather_info, { symbolize_names: true })
      stub_with_querys { |stub| stub.to_return(status: 200, body: weather_info) }

      result = client.check_weather(city_id)

      expect(result).to eq(weather_info_hash)
    end
  ...
  def stub_with_querys
    stub = stub_request(:get, 'https://api.openweathermap.org/data/2.5/weather')
           .with(query: {
                   'id' => city_id.to_s,
                   'units' => 'metric',
                   'appid' => ENV['API_KEY']
